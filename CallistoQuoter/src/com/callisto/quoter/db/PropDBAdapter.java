...
public class PropDBAdapter extends DBAdapter
{
	...
	static public final String T_PROPERTIES = "PROPERTIES",
			 ...
			 */
			C_RATING_ID = "re_rating_id", C_OWNER_URI = "re_owner_uri",
			C_LATITUDE = "re_latitude", C_LONGITUDE = "re_longitude",
			C_IMAGE = "re_image", C_PROP_TYPE_ID = "re_prop_type_id",
			// C_PRICE = "re_price",
	...
	public PropDBAdapter(Context context)
	{
		...
		{ C_ID, C_ADDRESS,
				...
				C_PROP_TYPE_ID,
				// C_PRICE,
				C_SURFACE_BUILT, C_SURFACE_PARCEL });
	}
	...
	{
		Cursor c = db.query(true, T_PROPERTIES, columns, null, null, null,
				null, null, null);
	...
	{
		Cursor c = db.query(true, T_PROPERTIES, columns, filter, null, null,
				null, null, null);
	...
	/***
	 * Retrieves all addresses and geolocation data from database, paired with
	 * property ID.
	 * 
	...
	public ArrayList<Address> getAllAddresses() throws SQLException
	{
		...
		while (c.moveToNext())
		{
			...
			double 
				lat = Double.parseDouble(String.valueOf(c.getFloat(c
					.getColumnIndexOrThrow(C_LATITUDE)))),
				lng = Double.parseDouble(String.valueOf(c.getFloat(c
						.getColumnIndexOrThrow(C_LONGITUDE))));

			t.setLatitude(lat);
			
			t.setLongitude(lng);

			t.setAddressLine(0, c.getString(c.getColumnIndexOrThrow(C_ADDRESS)));

			Bundle extras = new Bundle();

			Long id = c.getLong(c.getColumnIndexOrThrow(C_ID));
			Log.i(this.getClass().toString() + ".getAllAddresses",
					"Property id stored on address: " + id);
